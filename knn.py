...
# Writer Framework interface ========================================================
COLOR = {0:'#4444FF', 1:'#FF4444'}
def draw_graph(state):
    if 'knn' in state: state = state['knn']
    ...
    xs, ys, vs = knn.simdata.T.round(3)
    ...
    r = ['''Click in graph to select
a point for evaluation.''']
    click = state['click']
...
def add_nn(data, layout, result, knn, k, click):
    ...
    msg = f'''Point ({cx:0.1f},{cy:0.1f}) has
{k-count1} "B" and {count1} "R"
nearest neighbours.
So it is "{"BR"[cat]}".''' 
    result[0] = msg
